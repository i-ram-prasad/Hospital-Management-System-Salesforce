...
export default class ZodiacSignMessanger extends LightningElement {
    ...
    onChangeUserDateOfBirth(event){
        this.userDateOfBirth = event.target.value;
    }

    handleSubmit() {
        // console.log(`User name is: `+this.userName);
        // console.log(`User date of birth is: `+this.userDateOfBirth);
        const dob = new Date(this.userDateOfBirth);
        const userMonth = dob.getMonth()+1;
        console.log(`the month is ${userMonth}`);

        const userDay = dob.getDate();
        console.log(`the DAy is ${userDay}`);
        

        this.checkZodiacSign(userMonth, userDay);
    }
    
    checkZodiacSign(date, month){
